...
const createDispatch = async (req, res) => {
  try {
    const { requisition_id, facility_id, remarks, tracking_number, user_id } = req.body;

    // Use user_id from body if provided, otherwise fall back to req.user.id
    const dispatchedBy = user_id || (req.user && req.user.id);
    ...
    const [result] = await pool.execute(
      `INSERT INTO dispatches (requisition_id, facility_id, status, dispatched_by, tracking_number, remarks, created_at) 
       VALUES (?, ?, 'in_transit', ?, ?, ?, NOW())`,
      [requisition_id, facility_id, dispatchedBy, tracking_number || null, remarks || null]
    );
