...
const createRequisition = async (req, res) => {
  ...
  try {
    ...
    const { items, remarks, facility_id, priority = 'normal', user_id } = req.body;

    // Use user_id from request body instead of req.user
    if (!user_id) {
    ...
    const targetFacilityId = facility_id;
    ...
    const [requisitionResult] = await connection.execute(
      `INSERT INTO requisitions (user_id, facility_id, status, priority, remarks, created_at) 
       VALUES (?, ?, 'pending', ?, ?, NOW())`,
      [user_id, targetFacilityId, priority, remarks || null]
    );
...
const getRequisitionsByUser = async (req, res) => {
  ...
  try {
    // Fetch requisitions with user + facility info
    const [requisitions] = await pool.execute(
      `
      SELECT r.*, 
             u.name AS user_name, u.email AS user_email, u.phone AS user_phone,
